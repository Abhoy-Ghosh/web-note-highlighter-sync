/* Sticky Palette Highlighter
   - Palette stays open until Save/Cancel
   - 3 colors + Transparent
   - Note input + Save/Cancel
   - Hover delete UI
   - Stores {id,text,color,note,url,xpath,timestamp} in chrome.storage.local
*/
...
(function(){
  ...
  saveBtn.addEventListener('click', () => {
    ...
    if(wrap){
      const xpath = xpathForElement(wrap.node);
      const meta = { id: wrap.id, text, color: 'transparent', note, url: window.location.href, xpath, timestamp: new Date().toISOString() };
      persistHighlight(meta);
    }
    hideUI();
    window.getSelection().removeAllRanges();
